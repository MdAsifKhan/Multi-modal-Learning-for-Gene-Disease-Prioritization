...
def getGeneOntology = {
  ...
  GDataConf goConf = new GDataConf(GFormat.RDF_XML, "../../../Documents/multimodel_data/data/phenomNet/for-maxat-inferred.owl")
...
// def outclasses = new PrintWriter(new BufferedWriter(
//   new FileWriter('../../../Documents/multimodel_data/data/phenomNet_classes.txt')))
//   for (int i=0; i < phenomNet_classes.size(); i++){
//     outclasses.println(phenomNet_classes[i])
//   }
// outclasses.flush()
// outclasses.close()
  return graph
...
def getGenes = {
  def genes = []
  def i = 0
  new File("../../../Documents/multimodel_data/data/human_genes_mouse_phen_tab1.txt").splitEachLine('\t') { items ->
    def s = 0
...
//human_genes_mouse_phen
def dis_names = []
def getDiseases = {
  def dis = []
  def i = 0
  new File("../../../Documents/multimodel_data/data/omim_diseases_human_phens_tab1.txt").splitEachLine('\t') { items ->
    def s = 0
...
def fout = new PrintWriter(new BufferedWriter(
  new FileWriter("../../../Documents/multimodel_data/data/sim_scores1.txt")))
for (i = 0; i < result.size(); i++) {
  def x = i.intdiv(diseases.size()) //
  def y = i % diseases.size()
...
def disout = new PrintWriter(new BufferedWriter(
  new FileWriter('../../../Documents/multimodel_data/data/diseases1.txt')))
...
def genout = new PrintWriter(new BufferedWriter(
  new FileWriter('../../../Documents/multimodel_data/data/genes1.txt')))
